    ...
    <div className="pt-32 min-h-screen p-10 bg-linear-to-b from-[#faf6ef] to-[#f0e5d2] ">
      ...
      {error && (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="flex items-center gap-2 p-3 bg-red-100 border border-red-300 text-red-700 rounded-md mb-6"
        >
          <AlertCircle size={20} />
          {error}
        </motion.div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {products.map((p) => (
            <motion.div
              ...
              className="p-6 bg-white/90 backdrop-blur-xl border border-[#C9A86A]/25 rounded-2xl shadow-md hover:shadow-2xl cursor-pointer transition-all "
            >
